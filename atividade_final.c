...
QueueHandle_t structQueue_translation = NULL; 
QueueHandle_t structQueue_translated = NULL; 
QueueHandle_t structQueue_led_blinker = NULL; 
...
static void prvTask_getChar(void *pvParameters)
{
    ...
    for (;;)
    {
        ...
        {   
            ...
            else if (!isdigit(key) && !isalpha(key) && key != ' '){ // Filtrando digitos, numeros e espaco
                gotoxy(1,6);
                printf("Somente caracteres, sendo letras, numeros e espaco. \n");
...
static void prvTask_translator (void *pvParameters)
{
    ...
    for (;;)
    {
        ...
        for (int i=0; i< strlen(morse_character); i++){
            ...
            {
                gotoxy(1,6);
                printf("Alerta: Erro de memoria. Parte da MSG nao transmitida", morse_character); 
...
void app_run(void)
{
    ...
    structQueue_translation = xQueueCreate(10, 
                                           1); 
    ...
    structQueue_translated = xQueueCreate(10, 
                                           1); 
    ...
    structQueue_led_blinker = xQueueCreate(10, 
                                           1); 
